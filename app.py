...
def detect_columns(headers):
    ...
    for h, nh in norms.items():
        if mapping["feedback"] is None and (
            "feedback" in nh or "comment" in nh or "remark" in nh
        ):
            mapping["feedback"] = h
        if mapping["label"] is None and (
            "label" in nh or "sentiment" in nh or "rating" in nh
        ):
            mapping["label"] = h
        if mapping["teacher"] is None and "teacher" in nh:
            mapping["teacher"] = h
        if mapping["facility"] is None and (
            "facility" in nh or "laboratory" in nh or "library" in nh
        ):
            mapping["facility"] = h
...
def plot_interactive(series, title, filename_html):
    ...
    fig = px.bar(
        ...
        orientation="h",
        title=title,
        labels={"value": "Positive ratio", "label": ""},
    )
...
def plot_grouped_pos_neg(pos_series, title, filename_html):
    ...
    df = _pd.DataFrame(
        {"label": list(pos_series.index), "Positive": list(pos_series.values)}
    )
    df["Negative"] = 1.0 - df["Positive"]
    long = df.melt(
        id_vars="label",
    ...
    fig.update_layout(
        xaxis={"categoryorder": "total descending"}, yaxis=dict(range=[0, 100])
    )
    out_path = os.path.join(app.config["GENERATED_FOLDER"], filename_html)
    pio.write_html(fig, file=out_path, include_plotlyjs="cdn", full_html=True)
    return out_path
...
def process_dataframe(
    ...
    if teacher_col and teacher_col in df.columns:
        teacher_group = (
            df.groupby(teacher_col)["predicted"].mean().sort_values(ascending=False)
        )
        tfile = f"teacher_{uuid.uuid4().hex}.html"
        tpath = plot_grouped_pos_neg(
            teacher_group, "Teacher positive vs negative (%)", tfile
        )
        ...
        tover_path = plot_overall_pos_neg(
            t_positive, t_negative, "Teacher overall Positive vs Negative", tover_file
        )
        agg_results["teacher_overall_plot"] = tover_file
    if facility_col and facility_col in df.columns:
        facility_group = (
            df.groupby(facility_col)["predicted"].mean().sort_values(ascending=False)
        )
        ffile = f"facility_{uuid.uuid4().hex}.html"
        fpath = plot_grouped_pos_neg(
            facility_group, "Facility positive vs negative (%)", ffile
        )
        ...
        fover_path = plot_overall_pos_neg(
            f_positive, f_negative, "Facility overall Positive vs Negative", fover_file
        )
    ...
    if branch_col and branch_col in df.columns:
        branch_group = (
            df.groupby(branch_col)["predicted"].mean().sort_values(ascending=False)
        )
        bfile = f"branch_{uuid.uuid4().hex}.html"
        bpath = plot_grouped_pos_neg(
            branch_group, "Branch positive vs negative (%)", bfile
        )
    ...
    if department_col and department_col in df.columns:
        dept_group = (
            df.groupby(department_col)["predicted"].mean().sort_values(ascending=False)
        )
        dfile = f"department_{uuid.uuid4().hex}.html"
        dpath = plot_grouped_pos_neg(
            dept_group, "Department positive vs negative (%)", dfile
        )
    ...
    opath = plot_overall_pos_neg(
        positive, negative, "Overall Positive vs Negative", ofile
    )
